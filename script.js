...
productsList.map((item) => {
  ...
  element.innerHTML = `<div class="border rounded border-2 border-warning h-100 p-3">
                    <div class="images d-flex justify-content-center"></div>
  ...
  document.querySelector(`#item-${item.id} .price`).innerHTML =
    item.price + "$";
  document.querySelector(`#item-${item.id} .description`).innerHTML =
    item.description;
  ...
  item.images.map((image) => {
    ...
    document
      .querySelector(`#item-${item.id} .images`)
